...
<div class="flex justify-between items-center mb-8">
    <h1 class="text-2xl font-bold text-slate-800 dark:text-white transition-colors duration-300">Data Pegawai</h1>
    ...
    <table class="w-full text-left">
        ...
        <tbody class="divide-y divide-slate-50 dark:divide-slate-700/50">
            ...
            <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition border-b border-slate-50 dark:border-slate-700/50 last:border-none">
                <td class="px-6 py-4 font-semibold text-slate-800 dark:text-white">{{ $p->nip }}</td>
                <td class="px-6 py-4 text-slate-800 dark:text-slate-200">{{ $p->nama }}</td>
                <td class="px-6 py-4 text-slate-500 dark:text-slate-400">{{ $p->jabatan->nama_jabatan ?? '-' }}</td>
                <td class="px-6 py-4 text-slate-500 dark:text-slate-400">{{ $p->pangkat }}</td>
                <td class="px-6 py-4 text-slate-500 dark:text-slate-400">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800 dark:bg-indigo-500/20 dark:text-indigo-400">
                        {{ $p->golongan }}
                    </span>
                </td>
