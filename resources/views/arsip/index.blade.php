...
@section('content')
<div class="mb-8">
    <h1 class="text-3xl font-bold text-slate-800 dark:text-white transition-colors duration-300">Data Arsip Pegawai</h1>
    <p class="text-slate-500 dark:text-slate-400 mt-2 transition-colors duration-300">Kelola input nilai objektif (Pendidikan, Masa Kerja, SKP, Disiplin) untuk masing-masing pegawai.</p>
</div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                    ...
                    <th class="py-4 px-6 font-semibold text-slate-600 dark:text-slate-300 text-sm border-b border-slate-100 dark:border-slate-700/50">Pendidikan</th>
                    <th class="py-4 px-6 font-semibold text-slate-600 dark:text-slate-300 text-sm border-b border-slate-100 dark:border-slate-700/50">Masa Kerja</th>
                    <th class="py-4 px-6 font-semibold text-slate-600 dark:text-slate-300 text-sm border-b border-slate-100 dark:border-slate-700/50">SKP</th>
                    <th class="py-4 px-6 font-semibold text-slate-600 dark:text-slate-300 text-sm border-b border-slate-100 dark:border-slate-700/50">Disiplin</th>
            ...
            <tbody>
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors group">
                    ...
                    <td class="py-4 px-6 border-b border-slate-100 dark:border-slate-700/50 text-slate-600 dark:text-slate-400">
                        {{ $pegawai->arsip->nilai_pendidikan ?? '-' }}
                    </td>
                    <td class="py-4 px-6 border-b border-slate-100 dark:border-slate-700/50 text-slate-600 dark:text-slate-400">
                        {{ $pegawai->arsip->nilai_masa_kerja ?? '-' }}
                    </td>
                    <td class="py-4 px-6 border-b border-slate-100 dark:border-slate-700/50 text-slate-600 dark:text-slate-400">
                        {{ $pegawai->arsip->nilai_skp ?? '-' }}
                    </td>
                    <td class="py-4 px-6 border-b border-slate-100 dark:border-slate-700/50 text-slate-600 dark:text-slate-400">
                        {{ $pegawai->arsip->nilai_disiplin ?? '-' }}
                    </td>
                    ...
                    <td class="py-4 px-6 border-b border-slate-100 dark:border-slate-700/50 text-center">
                        <a href="{{ route('arsip.edit', $pegawai->id) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-600 hover:text-white dark:hover:bg-indigo-600 dark:hover:text-white rounded-lg transition-all duration-300 font-medium text-xs">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                            </svg>
                            {{ $pegawai->arsip ? 'Edit' : 'Input' }}
                        </a>
                    </td>
                </tr>
                @endforeach
