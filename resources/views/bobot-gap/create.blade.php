...
<div class="mb-6 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-slate-800 dark:text-white transition-colors duration-300">Tambah Bobot GAP</h1>
        <p class="text-slate-500 dark:text-slate-400 mt-1 transition-colors duration-300">Tambahkan konversi nilai selisih kompetensi baru.</p>
    </div>
...
<div class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700/50 shadow-sm p-8 rounded-3xl max-w-xl transition-colors duration-300">
    <form action="{{ route('bobot-gap.store') }}" method="POST" class="space-y-6">
        @csrf
        
        <!-- Selisih Input -->
        <div>
            <label for="selisih" class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Nilai Selisih (GAP)</label>
            <input type="number" step="0.01" name="selisih" id="selisih" value="{{ old('selisih') }}"
